#!/usr/bin/env python
"""
train_unet_2channel.py

U-Net training for RFI semantic segmentation using 2-CHANNEL INPUT.

Input representation: [real, imag] directly from complex data
- Channel 0: Real part (normalized by 99th percentile)
- Channel 1: Imaginary part (normalized by 99th percentile)

This is the training script that produced model/best_model.pth.

For 4-channel training (magnitude dB, cos/sin phase, valid), see train_unet_4channel.py.
...
# Import UNet architecture from centralized module
...
def main(args):
    ...
    # Model (fixed architecture for fair comparison with 4-channel)
    UNET_FEATURES = [64, 128, 256, 512]
    print(f"Initializing U-Net with features={UNET_FEATURES}...")
    model = UNet(in_channels=2, out_channels=1, features=UNET_FEATURES).to(device)
