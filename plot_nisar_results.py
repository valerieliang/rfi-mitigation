...
def main():
    ...
    print(f"Total CPIs: {num_cpi}")
    print(f"Clean (knee=0): {n_clean} ({100*n_clean/num_cpi:.1f}%)")
    print(f"RFI detected (knee>0): {n_rfi} ({100*n_rfi/num_cpi:.1f}%)")
    print(f"  Recoverable (knee≤{args.knee_bound}): {n_recoverable} ({100*n_recoverable/num_cpi:.1f}%)")
    print(f"  Severe (knee>{args.knee_bound}): {n_severe} ({100*n_severe/num_cpi:.1f}%)")
    ...
    clean_indices = np.where(knee_indices == 0)[0]
    rfi_indices = np.where(knee_indices > 0)[0]
    ...
    ax1.plot(knee_indices, 'o', markersize=2, alpha=0.5)
    ...
    ax2.plot(confidences, 'o', markersize=2, alpha=0.5, color='green')
    ax2.axhline(np.mean(confidences), color='red', linestyle='--', linewidth=2,
                label=f'Mean={np.mean(confidences):.3f}')
    ax2.set_xlabel('CPI Index', fontsize=11)
    ...
    if is_spatial:
        ...
        # Original predictions
        im1 = ax1.imshow(knee_indices.T, aspect='auto', cmap=cmap,
                         vmin=0, vmax=args.knee_bound, origin='lower')
        ax1.set_xlabel('Pulse Tile Index', fontsize=11)
        ax1.set_ylabel('Range Tile Index', fontsize=11)
        ...
        # Bounded predictions
        im2 = ax2.imshow(knee_bounded.T, aspect='auto', cmap=cmap,
                         vmin=0, vmax=args.knee_bound, origin='lower')
        ax2.set_xlabel('Pulse Tile Index', fontsize=11)
        ax2.set_ylabel('Range Tile Index', fontsize=11)
        ...
        im = ax.imshow(confidences.T, aspect='auto', cmap='viridis',
                       vmin=0, vmax=1, origin='lower')
        ax.set_xlabel('Pulse Tile Index', fontsize=11)
        ax.set_ylabel('Range Tile Index', fontsize=11)
