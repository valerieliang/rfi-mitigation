...
def score_channel(raw, freq, pol, model, args):
    ...
    # Only support the baseline 2-input model: [eigen, global]
    # The model from model.py with 3 global features.
    if len(model.inputs) != 2:
        raise ValueError(f"Unsupported model with {len(model.inputs)} inputs; "
                         f"expected 2 [eigen_input, global_input].")

    n_global_model = int(model.inputs[-1].shape[-1])
    if n_global_model != 3:
        raise ValueError(
            f"Model expects {n_global_model} global features; "
            f"this script only supports the baseline model with 3 global features.")

    # Allocate feature arrays for the baseline model
...
def save_predictions_h5(rec, args, out_dir):
    ...
    with h5py.File(path, 'w') as f:
        ...
        # The 3 global features fed to the baseline model
