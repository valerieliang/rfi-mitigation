#!/usr/bin/env python
"""
train_unet_4channel.py

U-Net training for RFI semantic segmentation using 4-CHANNEL INPUT.

Input representation: [magnitude_db, cos_phase_diff, sin_phase_diff, valid]
...
class RFIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.cached:
            ...
            valid = self.valid[idx].astype(np.bool_)
        else:
            ...
            with h5py.File(self.h5_paths[file_idx], 'r') as f:
                ...
                valid = f['valid'][sample_idx].astype(np.bool_)

        # Convert complex to 4-channel input [mag_db, cos_phase, sin_phase, valid]
        tile_4ch = build_input_channels(tile, valid, n_channels=4)

        # To tensors
        tile_t = torch.from_numpy(tile_4ch)
        mask_t = torch.from_numpy(mask).unsqueeze(0)
        valid_t = torch.from_numpy(valid.astype(np.float32)).unsqueeze(0)
