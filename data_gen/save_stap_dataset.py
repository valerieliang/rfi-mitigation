...
Usage
-----
    # Default: 1000 samples, JNR 10-30 dB
    python save_stap_dataset.py

    # Custom JNR range and sample count
    python save_stap_dataset.py --n 500 --jnr-min 5 --jnr-max 20

    # Full options
    python save_stap_dataset.py \\
        --n 1000 --K 128 --M 16 \\
        --noise 3.0 --signal 9.0 \\
        --jnr-min 10 --jnr-max 30 \\
        --frac 0.5 --seed 42 \\
        --out stap_dataset.h5
...
------------
    ...
    with h5py.File("stap_dataset.h5", "r") as f:
...
from gen_stap_dataset import generate_dataset, iter_dataset
...
def save_dataset(
    ...
    jnr_min_db: int = 10,
    jnr_max_db: int = 30,
    contaminated_fraction: float = 0.8,
    base_seed: int = 0,
    chunk_size: int = 64,
    verbose: bool = True,
) -> None:
    ...
    Samples are written in chunks of `chunk_size` to avoid holding the full
    dataset in memory.  Progress is printed every chunk when verbose=True.
    ...
    with h5py.File(out_path, "w") as f:
        ...
        def _flush(buf: dict, write_start: int) -> None:
            n = len(buf["label"])
        ...
        for sample in gen:
            ...
            buf["rfi_style"].append(
                (sample.rfi_style or "").encode()
            )
            buf["jnr_db"].append(
...
def verify_h5(path: str) -> None:
    """Print a brief summary of the HDF5 file contents."""
    with h5py.File(path, "r") as f:
        n          = f["data/contaminated"].shape[0]
        ...
        print(f"  datasets      : {list(f['data'].keys())}")
...
def _parse() -> argparse.Namespace:
    ...
    p.add_argument("--K",       type=int,   default=128,                  help="Range bins per image")
    p.add_argument("--M",       type=int,   default=16,                   help="Pulses per image (SCM dim)")
    ...
    p.add_argument("--jnr-min", type=int,   default=10,                   help="JNR lower bound (dB, inclusive)")
    p.add_argument("--jnr-max", type=int,   default=30,                   help="JNR upper bound (dB, inclusive)")
    p.add_argument("--frac",    type=float, default=0.8,                  help="Contaminated fraction [0, 1]")
    p.add_argument("--seed",    type=int,   default=0,                   help="Base RNG seed")
    p.add_argument("--chunk",   type=int,   default=64,                   help="Write chunk size")
    p.add_argument("--out",     type=str,   default="stap_dataset.h5",    help="Output HDF5 file path")
    p.add_argument("--verify",  action="store_true",                      help="Run read-back check after writing")
    return p.parse_args()
